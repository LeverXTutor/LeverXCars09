...
function carCreate(param){
	...
	//Get Next Car Number
	pStmt = param.connection.prepareStatement("select \"LeverXCars09.core_db.sequences::carId\".NEXTVAL from dummy"); 
	...
	for( var i = 0; i<2; i++){
		var pStmt;
		if(i<1){
			pStmt = param.connection.prepareStatement("insert into \"LeverXCars09.core_db.data::core_model.Car\" values(?,?,?,?,?,?)" );			
		}else{
